<!DOCTYPE html>
<html lang="ru">

<?php include ("inc/header.php"); ?>
	<?php include ("inc/menu.php"); ?>

		<body id="body">
			<div id="home_block">
				<div class="home_main" id="home_main_1">
					<div class="home_main" id="home_main_2"></div>
				</div>
				<div class="main_text">
					Сайт разработчика
					<div class="main_text_sub">
						Кодер BlackRockSoul. Найдешь что-нибудь интересное тут?
						<span class="main_button">
							<a href="#" onclick="moveTo_block(0)">Узнать больше</a>
						</span>
					</div>
				</div>
			</div>
			<div class="sub_block1 blck_set blck1" id="bl1">
				<div class="sub_text1_1 sub_set">
					<img src="/images/photo.png" alt="BlackRockSoul" class="photo_img">
				</div>
				<div class="sub_text1 sub_set">
					<h1 style="display: block">BlackRockSoul</h1>
					<div class="sub_text_block">
						<hr>
						<h2>Зовут меня Александр, мне что-то около 18 лет. В свободное время пишу различные странные вещи по типу этого сайта. Просто для души. Знание языков так себе, но с обучением проблем нет. Главное чтобы скучно не было. А вообще стараюсь что-то делать на <strong>c#</strong>, <strong>Delphi</strong>, <strong>html</strong>, <strong>php</strong>, <strong>js</strong> и других. Всего по немногу.</h2>
						<hr>
					</div>
				</div>
			</div>
			<div class="sub_block1 blck_set blck2" id="bl2">
				<div class="sub_text2_1 sub_set">
					<h1>Проекты</h1></div>
				<div class="sub_text2 sub_set">
					<div>
						<h2>Content 1</h2></div>
					<div>
						<h2>Content 2</h2></div>
					<div>
						<h2>Content 3</h2></div>
				</div>
			</div>
			<div class="sub_block1 blck_set blck3" id="bl3">
				<div class="sub_text3_1 sub_set">
					<img src="/images/photo_team.png" alt="Defend" class="photo_img">
				</div>
				<div class="sub_text3 sub_set">
					<h1 style="display: block">Команда</h1>
					<div class="sub_text_block">
						<hr>
						<h2>Лучший и единственный мой помощник - Никита. Почти все проекты начинаем или заканчиваем мы в итоге вместе. И даже если изначально это был сольный проект, то это меняется очень быстро. Особо много писать не буду, Вы и сами можете почитать о нем на его сайте - <a href="http://defdev.ru/" target="_blank" style="text-decoration: underline; color: white;">defdev.ru</a></h2>
						<hr>
					</div>
				</div>
			</div>
			<div class="sub_block1 blck_set blck4" id="bl4">
				<div class="sub_text4 sub_set">
					Обратная связь тут будет
					<br> Хорошая идея?
				</div>
			</div>
			<div class="sub_block1 blck_set blck5" id="bl5">
				<div class="sub_text5 sub_set">
					<h1>Нижнее меню</h1>
					<h2>Многовато места наверное оставил... Но, думаю сойдет.</h2>
				</div>
			</div>
		</body>

</html>
